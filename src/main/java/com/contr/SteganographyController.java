...
public class SteganographyController {
    ...
    public ResponseEntity<byte[]> encodeImage(
        ...
        byte[] result = imageService.encode(file, message, password);
    ...
    public ResponseEntity<String> decodeImage(
        ...
        try {
            String decoded = imageService.decode(file, password);
            ...
            if ("WRONG_PASSWORD".equals(e.getMessage())) {
                return ResponseEntity.status(403).body("WRONG_PASSWORD");
            }
            throw e;
        }
    }
    ...
    public ResponseEntity<byte[]> encodeAudio(
            @RequestParam MultipartFile file,
            @RequestParam String message,
            @RequestParam(required = false, defaultValue = "") String password) throws Exception {

        byte[] result = audioService.encode(file, message, password);
    ...
    public ResponseEntity<String> decodeAudio(
            @RequestParam MultipartFile file,
            @RequestParam(required = false, defaultValue = "") String password) throws Exception {

        try {
            String decoded = audioService.decode(file, password);
    ...
    public ResponseEntity<byte[]> encodeVideo(
            @RequestParam MultipartFile file,
            @RequestParam String message,
            @RequestParam(required = false, defaultValue = "") String password) throws Exception {

        byte[] result = videoService.encode(file, message, password);
    ...
    public ResponseEntity<String> decodeVideo(
            @RequestParam MultipartFile file,
            @RequestParam(required = false, defaultValue = "") String password) throws Exception {

        try {
            String decoded = videoService.decode(file, password);
